...
export async function GET(req: NextRequest) {
  ...
  if (type === "medicos") {
    ...
    data = [[t("reports.export.doctor"), t("reports.export.specialty"), t("reports.export.consultations"), t("reports.export.revenue", { currency })], ...doctors.map((d) => [d.name, d.specialty.name, cm.get(d.id) ?? 0, mzn(rm.get(d.id) ?? 0)])];
    ...
    data = [[t("reports.export.specialty"), t("reports.export.revenue", { currency })], ...rev.map((r) => [sm.get(r.specialtyId!) ?? "—", mzn(r._sum.amount ?? 0)])];
  } else if (type === "planos") {
    ...
    data = [[t("reports.export.insurer"), t("reports.export.plan"), t("reports.export.billed", { currency }), t("reports.export.received", { currency }), t("reports.export.outstanding", { currency })], ...inv.map((r) => {
    ...
    return NextResponse.json({ error: t("reports.export.invalidType") }, { status: 400 });
  }
